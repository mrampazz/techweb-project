...
if (filePath != "") {
    ...
    alert("Il tipo di file selezionato non \xE8 valido. Solo i formati JPG, JPEG e PNG sono supportati.");
    fileInput.value = '';
    return false;
    } else {
    ...
    if (fileInput.files && fileInput.files[0]) {
        ...
        document.getElementById($previewElemId).setAttribute("src", e.target.result);
        };
...
if (item.value == "") {
    ...
    case 'username':
        errorMessage = "Inserisci l'username.";
        break;
    ...
    case 'confirmation-password':
        errorMessage = "Inserisci la password di conferma.";
        break;
    }
} else if (isLogin) {
    switch (item.name) {
    case 'username':
        errorMessage = "L'username inserito non \xE8 corretto.";
        break;
    }
} else {
    switch (item.name) {
    case 'name':
        errorMessage = "Controlla il Nome! Il campo non può essere composto da valori numerici.";
        break;

    case 'surname':
        errorMessage = "Controlla il Cognome! Il campo non può essere composto da valori numerici.";
        break;

    case 'username':
        errorMessage = "Controlla l'username! Il campo non pu\xF2 essere composto da caratteri speciali.";
        break;
    ...
    case 'confirmation-password':
        errorMessage = "La password di conferma \xE8 diversa dalla password!";
        break;
    }
}
}
...
if (!RE_DATE.test(dateString)) return false; // Parse the date parts to integers
